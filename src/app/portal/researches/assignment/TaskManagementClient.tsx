...
export default function TaskManagementClient() {
  ...
  function normalizeTask(item: TaskSummaryResponse): UiTask {
    ...
    const yearlyPeriods = Array.isArray(item.periods)
      ? item.periods.map((p) => ({
          year: p?.yearNumber ?? 0,
          startDate: p?.startDate ? format(p.startDate as Date, 'yyyy.MM.dd') : '',
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [statusFilter, debouncedKeyword, setCurrentPage]);
  ...
  }, [statusFilter, debouncedKeyword, currentPage, itemsPerPage]);
        ...
        <div className="mb-10 grid grid-cols-1 gap-6 md:grid-cols-4">
          <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
            <div className="mb-3 flex items-center justify-between">
              <h3 className="text-sm font-semibold text-gray-700">전체 과제 수</h3>
