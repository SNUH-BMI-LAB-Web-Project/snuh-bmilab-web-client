...
const getProjectColumns = (
  ...
  },
  {
    ...
    cell: (row: ProjectSummary) => (
      <div>
        <Badge className={cn('whitespace-nowrap', getStatusClassName(row.status))}>
    ...
    cell: (row: ProjectSummary) => {
      ...
      const display =
        names.length > 1
          ...
          : names.join(', ');
      return (
        <div className="truncate overflow-hidden whitespace-nowrap">{display}</div>
      );
    },
  },
  {
  ...
  },
  {
    label: '실무 연구자',
    className: 'text-center w-[130px]',
    cell: (row: ProjectSummary) => <div>{`${row.participantCount ?? 0}명`}</div>,
  },
  {
    label: '연구 기간',
