...
import type { LeaveFindAllResponse, RejectLeaveRequest } from '../models/index';
import {
...
export class AdminLeaveApi extends runtime.BaseAPI {
   ...
   * 휴가 승인
   */
  async approveLeaveRaw(
    requestParameters: ApproveLeaveRequest,
    ...
    const response = await this.request(
      {
        path: `/admin/leaves/{leaveId}/approve`.replace(
          `{${'leaveId'}}`,
          encodeURIComponent(String(requestParameters['leaveId'])),
        ),
        method: 'POST',
   ...
   */
  async getLeaves1Raw(
    requestParameters: GetLeaves1Request,
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<runtime.ApiResponse<LeaveFindAllResponse>> {
    const queryParameters: any = {};
    ...
    return new runtime.JSONApiResponse(response, (jsonValue) =>
      LeaveFindAllResponseFromJSON(jsonValue),
    );
  }
   ...
   * 휴가 반려
   */
  async rejectLeaveRaw(
    requestParameters: RejectLeaveOperationRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<runtime.ApiResponse<void>> {
    if (requestParameters['leaveId'] == null) {
      throw new runtime.RequiredError(
    ...
    const response = await this.request(
      {
        path: `/admin/leaves/{leaveId}/reject`.replace(
          `{${'leaveId'}}`,
          encodeURIComponent(String(requestParameters['leaveId'])),
        ),
        method: 'POST',
...
export type GetLeaves1StatusEnum =
  (typeof GetLeaves1StatusEnum)[keyof typeof GetLeaves1StatusEnum];
