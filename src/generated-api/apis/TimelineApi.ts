...
import type { TimelineFindAllResponse, TimelineRequest } from '../models/index';
import {
...
export class TimelineApi extends runtime.BaseAPI {
  ...
  async createTimelineRaw(
    ...
    const response = await this.request(
      {
        path: `/projects/{projectId}/timelines`.replace(
          `{${'projectId'}}`,
    ...
    const response = await this.request(
      {
        path: `/projects/{projectId}/timelines/{timelineId}`
          ...
          .replace(
            `{${'timelineId'}}`,
            encodeURIComponent(String(requestParameters['timelineId'])),
          ),
        method: 'DELETE',
        headers: headerParameters,
        query: queryParameters,
   ...
   * 타임라인 첨부파일 삭제
   */
  async deleteTimelineFileRaw(
    requestParameters: DeleteTimelineFileRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<runtime.ApiResponse<void>> {
    if (requestParameters['projectId'] == null) {
      throw new runtime.RequiredError(
    ...
    const response = await this.request(
      {
        path: `/projects/{projectId}/timelines/{timelineId}/files/{fileId}`
          .replace(
            `{${'projectId'}}`,
            encodeURIComponent(String(requestParameters['projectId'])),
          )
   ...
   * 연구의 타임라인 기록 전체 조회
   */
  async getAllTimelinesByProjectIdRaw(
    requestParameters: GetAllTimelinesByProjectIdRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<runtime.ApiResponse<TimelineFindAllResponse>> {
    if (requestParameters['projectId'] == null) {
      throw new runtime.RequiredError(
        'projectId',
    ...
    return new runtime.JSONApiResponse(response, (jsonValue) =>
      TimelineFindAllResponseFromJSON(jsonValue),
    );
  }
   ...
   * 연구의 타임라인 기록 전체 조회
   */
  async getAllTimelinesByProjectId(
    requestParameters: GetAllTimelinesByProjectIdRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<TimelineFindAllResponse> {
    const response = await this.getAllTimelinesByProjectIdRaw(
      requestParameters,
      initOverrides,
    );
