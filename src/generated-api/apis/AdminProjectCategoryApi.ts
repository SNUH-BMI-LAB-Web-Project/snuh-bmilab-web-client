...
import type { ProjectCategoryRequest } from '../models/index';
import {
...
export class AdminProjectCategoryApi extends runtime.BaseAPI {
   ...
   * 연구 분야 추가
   */
  async createProjectCategoryRaw(
    requestParameters: CreateProjectCategoryRequest,
    ...
    const response = await this.request(
      {
        ...
        body: ProjectCategoryRequestToJSON(
          requestParameters['projectCategoryRequest'],
        ),
      },
      initOverrides,
    );
  ...
  async deleteByIdRaw(
    ...
    const response = await this.request(
      {
        path: `/admin/project-categories/{categoryId}`.replace(
          `{${'categoryId'}}`,
   ...
   * 연구 분야 수정
   */
  async updateProjectCategoryRaw(
    requestParameters: UpdateProjectCategoryRequest,
    initOverrides?: RequestInit | runtime.InitOverrideFunction,
  ): Promise<runtime.ApiResponse<void>> {
    if (requestParameters['categoryId'] == null) {
      throw new runtime.RequiredError(
