...
export default function ContractSubmissionFileSection({
  isEditMode,
  editData,
  setEditData,
  fileType,
  ...
  const handleAddFiles = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const before = editData?.[fileType]?.map((f: any) => f.fileId) ?? [];
      const all = [...before, ...uploaded.map((f) => f.fileId)];
      const patch = await fetch(`${API_BASE}/tasks/${taskId}/agreement`, {
        method: 'PATCH',
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
        ...
        body: JSON.stringify({
          [`${fileType.replace('Files', 'FileIds')}`]: all,
        }),
      });
      if (!patch.ok) throw new Error('파일 연결 실패');
      setEditData((p: any) => ({
        ...p,
        [fileType]: [...(p[fileType] || []), ...uploaded],
      }));
    } catch (err: any) {
  ...
  const handleDelete = async (fileId: string) => {
    ...
    setEditData((p: any) => ({
      ...p,
      [fileType]: (p[fileType] || []).filter((f: any) => f.fileId !== fileId),
    }));
  };
  ...
  const files = editData?.[fileType] || [];

  return (
    <div className="rounded-lg border border-gray-200 bg-white p-6">
      <h3 className="mb-4 text-lg font-semibold">최종 제출 파일</h3>
