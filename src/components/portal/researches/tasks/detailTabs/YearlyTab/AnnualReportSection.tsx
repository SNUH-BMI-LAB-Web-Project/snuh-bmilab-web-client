...
interface Props {
  ...
  taskId?: number;
  periodId?: number;
  onChange?: (update: { files?: FileMeta[] }) => void;
}
...
export default function AnnualReportSection({
  ...
  const handleAddFiles = async (
    ...
    try {
      ...
      const beforeIds = files.map((f) => f.fileId);
      ...
      await fetch(`${API_BASE}/tasks/${taskId}/periods/${periodId}`, {
        ...
        body: JSON.stringify({ annualReportFileIds: allIds }),
      });

      onChange?.({ files: [...files, ...uploadedResults] });
    } finally {
      e.target.value = '';
    }
  };
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-6">
      <h3 className="mb-4 text-lg font-semibold">연차보고서 ({year}년차)</h3>
