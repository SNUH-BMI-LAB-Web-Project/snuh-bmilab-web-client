...
export default function ProposalDiagramSection({
  isEditMode,
  editData,
  setEditData,
  taskId,
  ...
  const handleAddFiles = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (const file of Array.from(files)) {
        ...
        if (!pres.ok) throw new Error('Presigned URL 실패');
      ...
      const before =
        editData?.structureDiagramFiles?.map((f: any) => f.fileId) ?? [];
      ...
      const patch = await fetch(`${API_BASE}/tasks/${taskId}/proposal`, {
        ...
        body: JSON.stringify({ structureDiagramFileIds: all }),
      });
  ...
  const handleDelete = async (fileId: string) => {
    ...
    setEditData((p: any) => ({
      ...p,
      structureDiagramFiles: (p.structureDiagramFiles || []).filter(
  ...
  const files = editData?.structureDiagramFiles || [];

  return (
    <div className="rounded-lg border border-gray-200 bg-white p-6">
      <h3 className="mb-4 text-lg font-semibold">과제 구성 모식도</h3>
