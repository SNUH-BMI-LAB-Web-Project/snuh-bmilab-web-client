'use client';

import { useRef } from 'react';
import { Button } from '@/components/ui/button';
import { FileText, Trash2, Download } from 'lucide-react';

interface FileMeta {
  fileId: string;
  fileName: string;
  size: number;
  uploadUrl: string;
}

interface Props {
  isEditMode: boolean;
  editData: any;
  setEditData: (data: any) => void;
  taskId?: number;
}

export default function ProposalFinalFilesSection({
  isEditMode,
  editData,
  setEditData,
  taskId,
}: Props) {
  const inputRef = useRef<HTMLInputElement>(null);
  const API_BASE = process.env.NEXT_PUBLIC_API_BASE_URL;

  const getToken = () => {
    try {
      const raw = localStorage.getItem('auth-storage');
      if (!raw) return null;
      const parsed = JSON.parse(raw);
      if (typeof parsed.state === 'string')
        parsed.state = JSON.parse(parsed.state);
      return (
        parsed.state?.auth?.accessToken ||
        parsed.state?.accessToken ||
        parsed?.accessToken ||
        null
      );
    } catch {
      return null;
    }
  };

  const handleAddFiles = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const { files } = e.target;
    if (!files?.length) return;
    const token = getToken();
    if (!token) {
      alert('로그인이 필요합니다.');
      return;
    }

    try {
      const uploadedFiles: FileMeta[] = [];

      for (const file of Array.from(files)) {
        const presignedRes = await fetch(
          `${API_BASE}/files/presigned-url?fileName=${encodeURIComponent(
            file.name,
          )}&contentType=${encodeURIComponent(file.type || 'application/octet-stream')}`,
          { headers: { Authorization: `Bearer ${token}` } },
        );
        if (!presignedRes.ok) throw new Error('Presigned URL 요청 실패');
        const presignedJson = await presignedRes.json();
        const uuid =
          presignedJson.uuid ?? presignedJson.fileId ?? presignedJson.id;
        const presignedUrl = presignedJson.presignedUrl ?? presignedJson.url;
        if (!uuid || !presignedUrl) throw new Error('Presigned URL 응답 오류');

        const s3Res = await fetch(presignedUrl, {
          method: 'PUT',
          body: file,
          headers: { 'Content-Type': file.type || 'application/octet-stream' },
        });
        if (!s3Res.ok) throw new Error('S3 업로드 실패');

        const saveRes = await fetch(`${API_BASE}/files`, {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({
            uuid,
            fileName: file.name,
            extension: (file.name.split('.').pop() || '').toLowerCase(),
            size: file.size,
            taskId,
          }),
        });
        if (!saveRes.ok) throw new Error('파일 메타 저장 실패');
        const savedFile = await saveRes.json();

        uploadedFiles.push({
          fileId: savedFile.fileId ?? savedFile.id ?? savedFile.uuid,
          fileName: savedFile.fileName ?? file.name,
          size: savedFile.size ?? file.size,
          uploadUrl: savedFile.uploadUrl ?? savedFile.url,
        });
      }

      const beforeIds =
        editData?.finalProposalFiles?.map((f: FileMeta) => f.fileId) ?? [];
      const allIds = [...beforeIds, ...uploadedFiles.map((f) => f.fileId)];

      const patchRes = await fetch(`${API_BASE}/tasks/${taskId}/proposal`, {
        method: 'PATCH',
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ finalProposalFileIds: allIds }),
      });
      if (!patchRes.ok) throw new Error('파일 연결 실패');

      setEditData((prev: any) => ({
        ...prev,
        finalProposalFiles: [
          ...(prev.finalProposalFiles || []),
          ...uploadedFiles,
        ],
      }));
    } catch (err: any) {
      alert(`업로드 실패: ${err.message}`);
    } finally {
      e.target.value = '';
    }
  };

  const handleDeleteFile = async (fileId: string) => {
    const token = getToken();
    if (!token) return alert('로그인이 필요합니다.');
    const res = await fetch(`${API_BASE}/files/${fileId}`, {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${token}` },
    });
    if (!res.ok) return alert('삭제 실패');
    setEditData((prev: any) => ({
      ...prev,
      finalProposalFiles: (prev.finalProposalFiles || []).filter(
        (f: FileMeta) => f.fileId !== fileId,
      ),
    }));
  };

  const handleDownloadFile = async (file: FileMeta) => {
    const res = await fetch(file.uploadUrl);
    const blob = await res.blob();
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = file.fileName;
    a.click();
    URL.revokeObjectURL(a.href);
  };

  const files: FileMeta[] = editData?.finalProposalFiles || [];

  return (
    <div className="rounded-lg border border-gray-200 bg-white p-6">
      <h3 className="mb-4 text-lg font-semibold">최종 제안서 파일</h3>
      {!isEditMode && files.length === 0 && (
        <div className="text-sm text-gray-500">등록된 파일 없음</div>
      )}
      <div className="space-y-2">
        {files.map((f: FileMeta) => (
          <div
            key={f.fileId}
            className="flex items-center justify-between rounded-lg bg-gray-50 p-3"
          >
            <div className="flex items-center gap-2">
              <FileText className="h-4 w-4 text-gray-500" />
              <span>{f.fileName}</span>
              <span className="ml-3 text-sm text-gray-500">
                {Math.round(f.size / 1024)} KB
              </span>
            </div>
            <div className="flex items-center gap-2">
              <Button
                size="sm"
                variant="ghost"
                onClick={() => handleDownloadFile(f)}
              >
                <Download className="h-4 w-4 text-blue-600" />
              </Button>
              {isEditMode && (
                <Button
                  size="sm"
                  variant="ghost"
                  onClick={() => handleDeleteFile(f.fileId)}
                >
                  <Trash2 className="h-4 w-4 text-red-600" />
                </Button>
              )}
            </div>
          </div>
        ))}
      </div>
      {isEditMode && (
        <>
          <input
            ref={inputRef}
            type="file"
            multiple
            onChange={handleAddFiles}
            className="hidden"
          />
          <Button
            className="mt-3 bg-blue-600 text-white"
            onClick={() => inputRef.current?.click()}
          >
            파일 추가
          </Button>
        </>
      )}
    </div>
  );
}
