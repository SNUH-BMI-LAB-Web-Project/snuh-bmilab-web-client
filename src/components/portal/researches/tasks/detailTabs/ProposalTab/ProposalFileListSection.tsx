...
export default function ProposalFileListSection({
  isEditMode,
  editData,
  setEditData,
  taskId,
  ...
  const handleAddFiles = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (const file of Array.from(files)) {
        ...
        if (!pres.ok) throw new Error('Presigned 요청 실패');
        ...
        if (!save.ok) throw new Error('파일 메타 저장 실패');
      ...
      const before = editData?.relatedFiles?.map((f: any) => f.fileId) ?? [];
      ...
      const patch = await fetch(`${API_BASE}/tasks/${taskId}/proposal`, {
        ...
        body: JSON.stringify({ relatedFileIds: all }),
      });
      if (!patch.ok) throw new Error('파일 연결 실패');

      setEditData((p: any) => ({
        ...p,
        relatedFiles: [...(p.relatedFiles || []), ...uploaded],
      }));
    } catch (err: any) {
  ...
  const handleDelete = async (fileId: string) => {
    ...
    setEditData((p: any) => ({
      ...p,
      relatedFiles: (p.relatedFiles || []).filter(
  ...
  const files = editData?.relatedFiles || [];
